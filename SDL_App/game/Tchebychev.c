...
int main(int argc, char *argv[])
{
    ...
    window = SDL_CreateWindow("Tchebychev Groupe 3", 100, 100, WIDTH_WINDOW, HEIGHT_WINDOW, SDL_WINDOW_OPENGL);
...
{
    return fmax(abs(rect1->p->x - rect2->p->x) , abs(rect1->p->y - rect2->p->y));
}
...
void writeInFile(int nodesOpen, int nodesClose,struct timeval startTime, struct timeval endTime){
    ...
    file = fopen("Tchebychev.txt", "a+");
    fprintf(file, "--------------Distance Tchebychev------------------------------\n");
