...
int main(int argc, char* argv[]) {
    ...
    *(imgPlayers + 3) = loadTexture("SDL_App/down.png", renderer);
    ...
    imgEnd = loadTexture("SDL_App/monster.png", renderer);
    ...
    args->x1 = 0;
    args->y1 = 0;
    args->x2 = NBR_RECT_H - 1;
    args->y2 = NBR_RECT_V - 1;
    args->rects = rectsUI;
        ...
        if (SDL_PollEvent(&windowEvent)) {
            ...
            if (windowEvent.type == SDL_KEYDOWN) {
                switch (windowEvent.key.keysym.sym) {
                    case SDLK_1:
                        pthread_create(&monsterT, NULL, moveMonster, (void*)args);
                        args->state = 1;
                        break;
                    case SDLK_2:
                        args->state = 0;
                    ...
                    case SDLK_LEFT:
                        ...
                        break;
                    case SDLK_RIGHT:
                        if (x == NBR_RECT_H - 1 || (rectsUI[x + 1][y])->type == '1') {
                    ...
                    case SDLK_UP:
                        ...
                        break;
                    case SDLK_DOWN:
                        if (y == NBR_RECT_H - 1 || (rectsUI[x][y + 1])->type == '1') {
                            if (Mix_PlayingMusic() == 0) {
                                if (Mix_PlayMusic(music, 1) == -1) {
                                    return 1;
                                }
...
void* moveMonster(void* arg) {
    Mix_Music *music1, *music2;
    
    music1 = Mix_LoadMUS("SDL_App/step.wav");
    music2 = Mix_LoadMUS("SDL_App/over.wav");

    Args* args;
    args = (Args*)arg;
    ListPosition* path = A_Start(&((args->rects)[args->x2][args->y2]), &((args->rects)[args->x1][args->y1]));
    while (1) {
        args = (Args*)arg;
        ...
        if (args->state == 2) {
            args->state = 1;
            path = A_Start(&((args->rects)[args->x2][args->y2]), &((args->rects)[args->x1][args->y1]));
        }

        if (path) {
            ((args->rects)[args->x2][args->y2])->type = '\0';
            args->x2 = path->p->x / (WIDTH_RECT);
            args->y2 = path->p->y / (HEIGHT_RECT);
            ((args->rects)[args->x2][args->y2])->type = '3';
            path = path->next;
